...
async def test_agent_shared_with_a_user_still_cannot_list_cron(env: Any) -> None:
    """Being allowed to *open* the agent is not permission to read its jobs.

    ``get_cron`` / ``patch_cron`` / ``run_now`` all answer 403 for a shared
    non-owner; list must agree instead of answering ``[]``, which reads as
    "this agent has no scheduled jobs".
    """
    from octop.infra.sharing import VISIBILITY_PRIVATE, AclEntry
    from octop.infra.sharing.service import SharingService

    c, srv, _alice_auth, bob_auth, aid = env
    alice_id = int(srv.services.user_repo.get_by_username("alice").id)
    bob_id = int(srv.services.user_repo.get_by_username("bob").id)
    SharingService(srv.services.db).apply_change(
        alice_id,
        "agent",
        aid,
        AclEntry(
            ...
            unit_key=None,
            version=0,
            grants=(("user", str(bob_id)),),
        ),
    )

    # The share really reaches bob — otherwise the 403 below would come from
    # the agent ACL gate and would prove nothing about cron.
    assert (await c.get(f"/api/agents/{aid}", headers=bob_auth)).status_code == 200
