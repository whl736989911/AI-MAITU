...
--
--   feature_runs       one row per run: status, current step, the pipeline to wait
--                      on, the definition frozen at start, and the run snapshot
...
--
-- SQLite boots apply this through migrate.py::_ensure_feature_step_runs_schema:
-- ``CREATE TABLE IF NOT EXISTS`` is idempotent, but the tables must also reach
-- databases whose watermark skipped 24.
