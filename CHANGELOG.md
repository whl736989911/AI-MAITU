...
- 企业知识库改为**单一逻辑知识库**：一个部署只有一个知识库，用户不再能新建第二个（`POST /api/knowledge-bases` 与前端「新建知识库」入口一并移除），改用 `GET /api/knowledge-bases/enterprise` 读取。`knowledge_bases` 去掉用户归属（`owner_user_id` 可为空表示系统所有）并加 `is_enterprise`，由数据库的部分唯一索引保证「只能有一个」；已有知识库原样保留、权限不变
- 数据源新增**外部文件夹类型**：`local`（服务器目录）与 `smb`（SMB 共享，直接连接而无需操作系统预挂载），支持连接账号、加密保存的密码、仅包含 / 排除匹配式与扫描间隔。`nfs` 仍不可用：本构建没有 NFS 连接器（无纯 Python 客户端），创建时会明确拒绝并说明原因，而不是存下一条永远跑不起来的数据源
...
- 解析产物升级为**统一文档结构**（设计 §6.2 / §3.4）：一次解析不只产出正文，还产出 `title`（文档自己声明的标题 → 第一个标题行 → 文件名）、`sections`（Markdown/HTML 标题、Word 标题样式段落、幻灯片标题）、`tables`（**保留表头、行列与来源位置**：工作表名、幻灯片号或 Word 表格序号，不再压成一行文本）、`pages`（PDF 页数与幻灯片张数，格式不说就不猜）。结构在解析成功后作为**派生内容**写入 `knowledge_documents.derived_json`（含解析器版本，供后续判断哪些文档需要重解析）；**正文不重复保存**，仍只存在分片里。文档列表与预览接口新增 `title`（预览另返回 `pages`/`sections`），知识库预览弹窗因此显示文档自己的标题，而不是只能用文件名
  - 统一结构里 §6.2 的 `images` 暂未产出：它要逐格式枚举内嵌图片，而当前只有 §6 的「必要时用视觉模型描述」会用到它，等该能力落地时一并做
- 新增**提取模板**（设计 §7，企业级资源，不属于单个知识库）：可定义字段（名称、类型、必填、提取说明；类型含 `text` / `string[]` / `date` / `number` / `amount` / `boolean` / `enum`）、整体提取指令，以及**适用的文件类型**（类型不符的文件即使被绑定指向也不会套用）。**编辑生成新版本而不覆盖**——v1 永远保留，将来每条提取结果都能记录产出它的模板版本（§7.3）
- 新增**模板绑定**（§7.4）：可绑定到数据源、数据源内的目录，或单个文件，支持扩展名、MIME 类型、文件名模式、正则四种条件；优先级为**文件级 > 目录级 > 数据源级**，同一层级有多个模板命中时**报冲突而不是随机挑一个**（`GET /api/extract-templates/resolve` 返回冲突的模板 id）。模板停用后不能再新建绑定、已绑定的也不再套用；**有绑定的模板不能删除**（409 并说明还有几处绑定），清除绑定后可删除（版本随之一并清除）
- 前端「知识库」页新增**提取模板面板**：模板列表（状态 / 当前版本 / 字段数 / 绑定数）、新建与编辑（字段编辑器，enum 才需要可选项）、启用停用、删除，以及绑定管理——含「检查某路径会套用哪个模板、是否冲突」
- 迁移 **033** `knowledge_extract_templates` / `_versions` / `knowledge_extract_bindings` 三张表；模板版本用 `(template_id, version)` 唯一索引保证编号不重
- 新增**全文与关键词搜索**（设计 §9 / §12.9）：`GET /api/knowledge-bases/{kb}/search?q=` 在已索引的文档分片里做词法检索，**与向量检索相互独立**（对话召回走向量，这个接口走词法；两者互不依赖，也没有加部署级开关——§15 明确不做）。查询分词：拉丁字母与数字按词切，**中文按字种**——短串整串作为一个词（字段名、型号），长串切成两字窗口（一句话，用它去命中文档里真正出现的那一段）。排序三条信号：**整句出现** > **命中词覆盖率** > **命中次数（有上限，重复不再加分）**；同分时按文档与位置稳定排序，同一个查询不会每次给出不同顺序
- 每条结果都带**原文件路径与位置**（§14）：知识库内路径（或数据源内相对路径）、命中分片在文档中的序号、以及**以命中处为中心**截取的摘录（不是从分片开头截）；前端「知识库」页新增搜索框，结果可直接点开原文件预览
- 搜索的可见范围**复用对话召回那一条规则**（`list_visible` → `resource_acl`，含管理员直通），并且**只搜 `ready` 的文档**：正在索引、失败、待删除的文件搜不到，无权访问的知识库返回 403 —— 这是 §14「未授权内容不会出现在搜索…中」的落地
- **混合检索**（设计 §9 / §12.10）：对话召回不再只有向量那一路，而是**向量 + 词法两路并集**，用**倒数排名融合**（RRF，k=60）定序——两路都命中的分片拿到两份贡献、排在单一来源之前。不按分数加权，是因为两个分数没有共同量纲（余弦相似度 vs 词命中数），加权等于对不存在的单位做算术；RRF 也正是让不同知识库的结果能合并成一个上下文的原因。引用随之覆盖两路来源（引用仍按文档去重、保持召回顺序）
- 查询向量拿不到时（嵌入后端不可用），**词法那一路仍然回答这一轮**，而不是让对话拿不到任何知识——设计 §1 明确"向量模型是可选的语义检索能力，不负责文件解析和知识整理本身"。这种情况会记一条 warning，避免"悄悄失去嵌入能力"没人发现
- **提取执行**（设计 §12.6 / §7.3）：模板现在真的会跑——`POST /api/extract-templates/{id}/documents/{doc_id}` 用指定模板（或不指定时**按绑定规则自动解析**该文件该用哪个模板，§7.4）抽取字段，`POST /api/extract-templates/{id}/run` 按范围批量跑（可按目录限定、`only_failed` 只重试失败的、`only_stale` 跳过已用当前版本成功的）。结果写在**迁移 034** 的新表 `knowledge_extract_results`（一文档一模板一行，重跑原位替换），并记录 §7.3 要求的全部出处：**模板版本、模型、解析器版本、文件 hash、时间、状态**——所以"模板改过了，哪些结果过期了"是一个可回答的问题，而不是靠猜
- **模型回复不信任、按模板校验**：required 字段空着算**失败**（不是留个洞的结果）、枚举值不在选项内算**错答**（不是新增选项）、模板没声明的 key 会被丢掉；模型写成一行的列表会被拆成数组。失败原因会落到结果行上，行状态是 `failed` 而不是看起来像没跑过
- 抽取用的模型是**部署的当前活跃模型**（没有为它新增一个模型设置项），模型名随结果一起存下来；读文档正文走与索引**同一条**读取路径（数据源文件照样经连接器暂存临时副本，平台不复制原文件）
- 提取模板面板新增**「运行」与「结果」**两个动作：运行可限定范围（整个知识库或某个目录），可勾「只重试失败的」「只跑已过期的」，完成后报告命中/成功/失败/跳过；结果列表逐个显示文件、状态、**产出它的模板版本与模型**、字段值与失败原因。`GET /api/extract-templates/{id}/results` 随之提供这套数据（接口不返回原始 JSON 字符串，只给解析后的字段）
- 文档新增**内容摘要（SHA-256）**：上传、覆盖、以及数据源文件入库时都会计算并存入文档行。它回答"这个文件是被碰过还是被改过"——大小和修改时间回答不了这个问题。两个读者：提取结果的**过期判断**现在同时比对模板版本与内容摘要（文件内容变了，旧结果即过期），以及 §8.1 的比较清单有了对应字段
  - 诚实说明：我顺手做过一版"内容没变就不重新解析/嵌入"的优化，但它的测试显示**跳过没有按预期生效**，于是我把这段未经验证的分支**撤掉了**（没有留下会误导人的代码），只保留已验证的摘要存储与过期判断
...
- 共享（应用持有，`user_id IS NULL`）agent 的记忆文件不再被运行写入：这类 agent 没有主人，工作区的 `MEMORY.md` 是全体调用者共读的一份文件——张三跑一次留下的「记忆」，李四下次运行就会读到，也会被写回同一份文件。现在这类 agent 的轮次拒绝写工作区根目录的 `MEMORY.md`（`write_file` / `edit_file` / `append_file` / `delete`），记忆照常注入、可读；模型拿到的是一条说明原因的拒绝（不是静默无操作），服务端同时记一行日志。属于个人的 agent（含专家）行为完全不变
