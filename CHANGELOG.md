...
- 麦途智造的升级说明与 Release 下载链接改指向本项目 GitHub；本项目尚无 Release，控制台和 CLI 暂停从上游 PyPI 检查或安装 `octop`，避免一键升级把分叉版本替换成上游包。现使用 GitHub 源码目录的 `git pull --ff-only`、`uv sync` 和前端构建手动升级
