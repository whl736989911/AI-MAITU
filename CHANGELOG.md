...
- 移植上游多供应商媒体生成配置与探测：支持按实例配置图片、视频模型和默认路由，密钥只写且服务端仅允许官方 HTTPS 域名。
- 增加 Discord 通道配置与诊断、企查查 API Key 连接器、OpenAlex，以及记忆维护的控制台和命令入口。
- 团队专家支持名册、共享会话、成员协作及模板文件管理；创建专家时可编辑或移除预设人设文件，并从技能中心或其他专家复制技能。
- 对话新增 Ask、Plan、Craft 模式与按会话配置的人机确认策略；计划产物可在会话中确认后执行。
- 内置插件市场补齐十五种工具插件及图标、分组和交互卡片；沿用本项目首次安装后默认禁用的策略。

### 修复

- 聊天按专家与会话串行执行，清理已删除会话的检查点与过期 HITL 状态；ACP 外部 Runner 遵循沙箱开关。
- 工作区归档包含系统隐藏状态并阻断符号链接逃逸；修复 SSRF 地址解析与固定连接、OAuth 回调和验证码等安全边界。
- 控制台补齐离线提示、登录跳转、配置刷新、人机确认去重和移动端安全区域处理。
- 新建与编辑用户时可单独授予团队权限，默认不授予；团队管理同时要求有效的专家权限，取消或拒绝专家权限后团队权限不生效，前端导航与接口一致。


### 新增

- 功能记忆按工作流阶段隔离：草稿由作者手工训练共享记忆；启用后共享记忆对所有人只读，自动捕获与提炼仅写入已验证调用者自己的私有记忆。个性化页统一展示共享/私有来源和独立的个人指令；个人指令不从记忆生成，运行时优先于功能规则及记忆。

## [0.1.0] - 2026-09-23

麦途智造的首个独立版本。下方 1.x / 0.9.x 条目是上游 Octop 的历史版本，不属于本项目的版本序列。

### 新增

- 功能（`agents.kind = 'feature'`）新增**工作流定义**：功能 agent 工作区的 `.octop/workflow.json` 声明**输入表单**（字符串 / 数字 / 整数 / 开关 / 列表 / 文件上传，双语标题与说明）、**固定步骤**（名称、提示词、依赖的前序步骤、本步可用的技能 / 子智能体 / 工具、人工确认门）、**交付物**与**软规则**；`status` 分 `draft`（只校验形状，供作者训练和边写边存，其他调用者看不到）与 `active`（每个步骤必须有 id、名称与提示词，能访问功能的人才可读取）两档。接口为 `GET/PUT/DELETE /api/agents/{id}/workflow`——写沿用能力矩阵（功能作者可写、被授权者只读）；非法定义**一次列出全部问题**且一个字节都不落盘，专家用同一路径得到 `WORKFLOW_NOT_A_FEATURE` 而不是一份空定义
- 功能按声明**执行**：本轮请求的 `configurable` 携带定义快照与本次输入（功能行 + 工作区，在回合路径上一次读出，中间件不再逐次调用读盘），`FeatureWorkflowMiddleware` 把"本次输入 / 固定步骤 / 规则 / 交付物"追加到本轮 system message。注入只作用于模型调用，不写进 agent 持久配置、也不进入会话检查点；未声明工作流的功能仍会收到功能配置指引，仪表盘与 IM 两条入口拼装的是同一个上下文
- 功能新增**个人差异**（`feature_user_overlays`，迁移折叠进 `036`，按 `(feature_id, user_id)` 唯一）：调用者自己的文本，注入在功能定义**之后**并声明冲突时以它为准——使用者因此可以改内容，但改不到功能的配置，也读不到别人的差异
- 功能新增**运行记录**（`feature_workflow_runs`，同批迁移）：输入卡片每提交一次运行就写一条，含本次输入值与**当时的定义快照**（之后编辑工作流不会改写历史），`GET /api/agents/{id}/workflow/runs` 返回调用者自己的运行（别人的运行不列出）；功能被删除时其个人差异与运行记录一并清除，避免同 id 的新功能继承别人的文本
- 新增**改动记录与撤销**（`feature_workflow_changes`，同批迁移）：`POST /api/agents/{id}/workflow/changes` 以**差异**应用一次改进——每一项写清 `path`（如 `/rules/2`、`/steps/1/gate`，`-` 表示追加）、`before` 与 `after`。应用前逐项核对 `before` 是否仍等于当前值：**任何一项对不上就整批拒绝**（409 `WORKFLOW_CHANGE_CONFLICT`，一个字节不写），绝不覆盖之后有人做过的编辑；定义改动若会让定义不合法同样拒绝（400）。`POST .../changes/{id}/revert` 按同一份差异反向还原（改回的改回、新增的删除——包括新增的字段与追加的列表项），其中已被后续编辑改过的那一项**报出来而不是强推**，撤销后再撤销是幂等无操作。`GET .../changes` 只列出调用者自己应用的变更——作者的定义改动记录不会向其他人泄露草稿，个人差异的变更也只有本人可见
- 引用必须**真实存在**（**发布时才强制**）：`status: active` 的定义若在某一步引用了这个功能**没有**的技能或子智能体，保存会被拒（400）并**列出它实际有哪些**（`available: …`，一个都没装时直说），于是无论手改还是模型自纠都能一轮改对；`draft` 不受此限——正在训练的工作流本来就可能先写下一个"待安装"的技能。规则对**两条写路径都成立**（人按保存，以及由运行总结出来的改动把文档推到 `active` 时），因为两条路径都经过同一个 `save_definition`
- 功能新增**配置助手工具**（只装配给功能自己的 agent，共 5 个：`feature_workflow_get / save / change / revert / runs`）：作者在对话里说"第 3 步改成确认门"，agent 就能真的改；**差异式应用**（应用前逐项核对 `before`，对不上整批拒绝、一个字节不写）、**可撤销**、定义改动必须通过校验——与 HTTP 接口**同一份实现**（新增 `infra/agents/feature_workflow_service.py`，路由与工具共用，避免两份实现各自漂移，撤销与所应用的内容必然对得上）。工具从本轮自己的 agent 解析目标功能，只认**作者本人**：别人的功能或专家的 agent 一律拒绝，且不与 API 层重复实现权限规则
- 仪表盘「功能 → 工作流」新增可视化与 JSON 编辑、作者的草稿训练与首次启用确认；调用侧显示按输入定义生成的表单、结果卡片，以及可确认撤销的差异卡片。工作流页另有仅属于当前用户的个人指令、运行记录和改动历史；从草稿切换正式版后，其他被授权者才看到输入卡片
- 知识文档新增**文件级读取权限**：一个文件可以有自己的授权行（`resource_type = 'knowledge_document'`，是这套统一 ACL 的一个新资源类型），列表、搜索、预览、下载、正文读取与对话引用**同源**判断——文件没有自己的授权行时由所属知识库决定，而文件级规则**只能收窄、不能放宽**（design §5.2/§5.3/§14）。授权沿用既有的共享流程，因此不需要新的接口
- 旧的用户知识库**并入企业空间**（迁移 `035`，design §13）：文档、磁盘文件、**分片与向量索引（每库一个 SQLite 边车文件，`KnowledgeIndex.doc_chunks` 随之新增）**、数据源与 `agents.knowledge_base_ids` 的指向一并转移，**知识库原来的可见范围先写成每个文件的文件级授权行、再删除被搬空的库**——顺序是关键的：企业空间对全组织可见，若跳过这一步，原来只有少数人能看的文件会变成人人可见。迁移在 SQLite 与 PostgreSQL 上都执行同一套幂等步骤（SQL 无法搬文件，所以由 `migrate.py::_merge_legacy_knowledge_bases` 承担；它与版本分支一同执行，只在升级越过 v35 的那一次启动运行，重启不会再跑）
  - 未做（明确说明，不是遗漏）：design §13 的「旧上传文件**可以**作为一个兼容数据源导入」。平台把上传件存在 `<knowledge_dir>/<kb_id>/docs/<document_id><扩展名>`，以该目录做数据源会把「文档 id」当作文件名呈现给使用者；要做对需要先按原文件名改名（同库内重名需要另想办法），这一步没有做
  - 会丢失的只有旧库的**外壳**：名称、描述、图标、「默认打开」标记、每库的容量上限与嵌入模型记录（向量本身随分片一起搬走），以及它自己那条 ACL 行；文档、文件、分片与可见范围都不丢
- 企业知识库改为**单一逻辑知识库**：一个部署只有一个知识库，用户不再能新建第二个（`POST /api/knowledge-bases` 与前端「新建知识库」入口一并移除），改用 `GET /api/knowledge-bases/enterprise` 读取。`knowledge_bases` 去掉用户归属（`owner_user_id` 可为空表示系统所有）并加 `is_enterprise`，由数据库的部分唯一索引保证「只能有一个」；旧的用户知识库在 v35 迁移中并入企业空间（见下一条），可见范围随文件一起转移
...
- 功能 Agent 每轮在通用模板后追加按用户语言生成的功能配置指引：收到「训练功能」时应说明工作流、技能、子智能体、工具、插件、MBTI、记忆、通道、人设文件等入口，而不是只引导填写 USER.md / MEMORY.md；作者可继续描述具体改动，非作者只得到只读指引。功能的文件工具禁止写入共享根目录的 `USER.md` 与 `MEMORY.md`（专家仍可正常写），避免把个人资料误存到多人共用的功能工作区
- 麦途智造的升级说明与 Release 下载链接指向本项目 GitHub；控制台和 CLI 停用从上游 PyPI 检查或安装 `octop`，避免把分叉版本替换成上游包。当前使用 GitHub 源码目录的 `git pull --ff-only`、`uv sync` 和前端构建手动升级，不提供自动升级
- 工作区文件读写、下载、文档与媒体预览现在只接受当前 Agent 工作区内的路径，拒绝其他用户的 Agent 文件、宿主机临时文件及通过软链接逃逸的路径。直接位于 Desktop 等工作区外的工具产物不再提供下载或预览；需要先保存到该 Agent 工作区。
- 对外 HTTPS 请求只连接经验证的公网 IP；带尾点等主机名形式不再触发未固定的二次 DNS 解析，同时拒绝 `100.64.0.0/10` 等非公网地址。
...
- 专家卡片与表格的管理控件此前按「是不是我的」判断，与后端「所有者，或管理员」这条写入规则不一致：管理员在「我的专家」里本来就会看到别人私有的专家（`GET /agents` 返回 `owned ∪ list_visible`，而 `can_access` 对 `admin` 在第一句直通），但卡片只把 `is_owner` 当门，于是顶部启停开关与工作区 / 重载 / 编辑 / 删除 / 发布模板 / 更多整排被藏起，卡片上只剩专家 ID 与「对话」——而服务端对这些操作是会接受的（`api/common/agent.py` 的 `assert_agent_owner`：`user.is_admin` 直接返回）。现在卡片与表格共用 `utils/sharedExpert.ts` 的 `canManageExpert(agent, role)`：所有者，或系统管理员。对非管理员，这个判定与改动前逐字相同——「共享给他人（或按部门 / 角色授给他人）的专家，对非所有者仍是只读」这条不变；`enterprise_admin` / `unit_admin` 是受限管理员，不走这条直通
- 「功能」此前带一个自己的整页（`/features/:agentId`，上半是「功能定义」、下半是「能力配置」），删掉：它和专家那两个面是同一批组件——定义就是专家抽屉 `EditAgentDrawer` 编的那批字段，能力配置就是个性化页 / 卡片「更多」里的那组面板（`PersonalizationPanels`，功能作用域多一块「人设文件」）。同一个东西摆在两个入口、其中一个是没人主动打开的中间页，是多余的。随之删掉该页的路由（`/features/:agentId/*`）、页面与它的三个组件、只为它存在的 i18n 键，以及上一轮为它加的卡片「打开功能页」按钮（`AgentCard` 的 `pageAction`）。功能卡片现在只剩与专家卡片相同的两个动作：铅笔直接在列表上开抽屉（不再跳页），「更多」开能力面板；功能的人设文件仍在个性化页（选中该功能时它自带的 tab）；新建功能不再跳转，落在列表并刷新出它的卡片
