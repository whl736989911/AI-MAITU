...
- 功能按声明**执行**：本轮请求的 `configurable` 携带定义快照与本次输入（功能行 + 工作区，在回合路径上一次读出，中间件不再逐次调用读盘），`FeatureWorkflowMiddleware` 把"本次输入 / 固定步骤 / 规则 / 交付物"追加到本轮 system message。注入只作用于模型调用，不写进 agent 持久配置、也不进入会话检查点；未声明工作流的功能行为与之前逐字一致，仪表盘与 IM 两条入口拼装的是同一个上下文
